...
class _StoreViewAllTextStyles {
  ...
  static const productName = TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.black87);

  static const productPoints = TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFFFF5722));
}
...
class _StoreViewAllPageState extends State<StoreViewAllPage> {
              ...
              flexibleSpace: FlexibleSpaceBar(
                background: Stack(
                  ...
                  children: [
                    ...
                    Center(
                      child: AspectRatio(
                        aspectRatio: 3 / 2, // ✅ enforced ratio
                        child: CacheNetworkImageWidget(url: selected?.bannerUrl ?? '', fit: BoxFit.cover),
                      ),
                    ),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: _buildStoreDropdown(
                        ...
                        items: widget.stores.map((s) => MapEntry(s.id ?? '', s.name ?? '')).toList(growable: false),
                        onChanged: (value) {
            ...
            if (_loading)
              ...
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      Text('Error: $_error', textAlign: TextAlign.center, style: _StoreViewAllTextStyles.errorText),
                      const SizedBox(height: 12),
                      ElevatedButton(
                        onPressed: _fetchRewards,
                        style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFED5B23)),
                        child: const Text('Retry', style: _StoreViewAllTextStyles.retryButton),
                      ),
                    ],
                  ),
                ),
              ...
              const SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Center(child: Text('No rewards found', style: _StoreViewAllTextStyles.emptyText)),
                ),
              )
            else
              ...
              SliverPadding(
                ...
                sliver: SliverGrid(
                  gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                    crossAxisCount: 2,
                    childAspectRatio: 0.75, // same visual density
                    crossAxisSpacing: 12,
                    mainAxisSpacing: 12,
                  ),
  ...
  Widget _buildStoreDropdown({
    ...
    final safeValue = hasValue ? value : (items.isNotEmpty ? items.first.key : '');
  ...
  Widget _buildCategoryDropdown({required String value, required List<String> items, required ValueChanged<String?> onChanged}) {
    final safeValue = items.contains(value) ? value : (items.isNotEmpty ? items.first : items.firstOrNull ?? '');
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String?>(
          ...
          items: items.map((String item) {
            return DropdownMenuItem<String?>(
              value: item,
              child: Text(item, style: _StoreViewAllTextStyles.dropdownItem),
            );
          }).toList(),
          onChanged: onChanged,
        ),
  ...
  Widget _buildProductCard({required String name, required String imageUrl, required String points}) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(name, maxLines: 1, overflow: TextOverflow.ellipsis, style: _StoreViewAllTextStyles.productName),
                const SizedBox(height: 6),
                Text(points, style: _StoreViewAllTextStyles.productPoints),
              ],
            ),
          ),
        ],
